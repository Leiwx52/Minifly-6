...
void FTC_IMU::DCM_CF(Vector3f gyro,Vector3f acc, float deltaT)
{
	...
	angle.x = deltaT / 2 * (last_gyro.x + gyro.x);
	angle.y = deltaT / 2 * (last_gyro.y + gyro.y);
	angle.z = deltaT / 2 * (last_gyro.z + gyro.z);
	last_gyro = gyro;
	
	Matrix3f dcm;
	dcm.from_euler(angle);
	
	static Vector3f g(0, 0, 4096);
	static Vector3f x(200, 0, 0);
	g = dcm * g;
	x = dcm * x;
	
	//g = g*0.98 + acc*0.02;
	g = CF_1st(g, acc, ftc.factor.gyro_cf);
...
void FTC_IMU::Quaternion_CF(Vector3f gyro,Vector3f acc, float deltaT)
{
	...
	static Vector3f error_i;
	
	acc.normalize();
	
	Q.vector_gravity(gravity);
	
	error = acc % gravity;
	error_i += error * Ki;
	
	gyro += error * Kp + error_i;
